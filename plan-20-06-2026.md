...
Make sure to stick to best practices, design patterns, components, no hard-coding, separation of concerns and all that jazz, you get the drift. I don't want to see some beginner style coding. Anything we add must scale, in all the meanings of the word. Use native Web APIs only (Fetch API, standard DOM manipulation, local storage) and standard CSS. No external libraries or frameworks. This app's structure is using the modern component pattern in vanilla JS with standard ES modules and a pub sub model using proxies to manage reactive states and interaction.  Use a new GIT branch plan-20-06-2026-[actual time goes here] to implement your changes.